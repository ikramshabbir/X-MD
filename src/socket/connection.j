  ...
  conn.ev.on(  
    ...
    async (update) => {  
      ...
      if (  
        ...
        const safeDisconnectMessage =
          lastDisconnect?.error?.message ||
          lastDisconnect?.error?.output?.payload?.message ||
          "unknown";

        console.log(
          `🔎 Disconnect reason: ${safeDisconnectMessage}`
        );  

    const safeDisconnectData = lastDisconnect?.error?.data || {};

    console.log(
      `🔎 Server data: reason=${safeDisconnectData?.reason || "unknown"}, location=${safeDisconnectData?.location || "unknown"}`
    );
...
export async function requestPortalPairing(
  ...
  /* EXISTING ACTIVE PAIRING CODE */
  const existingPairing = pairingInfo.get(sessionId);

  if (
    existingPairing &&
    existingPairing.number === cleanNumber &&
    existingPairing.code &&
    existingPairing.connected === false
  ) {
    console.log(
      `♻️ Existing pairing code returned for ${sessionId}: ${existingPairing.code}`
    );
  ...
  try {
    ...
    const ready =
      pairingReadyPromises.get(sessionId);
